...
<div class="form-group" [ngClass]="{'has-danger': loginForm.get('password').touched && loginForm.get('password').invalid}">
  ...
  <input class="form-control" [ngClass]="{'form-control-danger': loginForm.get('password').touched && loginForm.get('password').invalid}" type="password" id="password" placeholder="Mot de passe" formControlName="password">
